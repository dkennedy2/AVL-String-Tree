...
public class GradingAppMain {															//This program makes and prints out a grading curve based on class scores entered by user. The program includes a calculator which can be skipped.
	...
	finalscore = sumtest + sumassignment;												//sums all scores to calculate the student's final score which is printed for the user
	System.out.println("The student's final score is " + finalscore);	

	System.out.println("Do you wish to sum a student's score's? Press 1 for yes and 2 for no.");		//Allows user to end the loop / calculator 
	sumloop = userinput.nextInt();
	}	
		
		System.out.println("Once the class size has been entered, the program will create a grading curve and print the new grading out at the end.");			
		System.out.println("Please enter the size of your class");										//Asks user the size of his/her class and creates and array of appropriate size. 
		...
		Arraydifference = Array;															//Code initializes Arrays used later
		Arraysquared = Arraydifference;
		...
		if (index == size) {																	//code runs if index of array equals size of class
			...
			for (int i = 0; i < Array.length; i++) 										//loops through array
				Arraydifference[i] = Array[i] - average;									//Subtracts the average from each value in the array and stores the value in Arraydifference
				
				
			for (int i = 0; i < Array.length; i++) 										//loops through array
				Arraysquared[i] = Math.pow(Arraydifference[i],2);							//Squares each value in Arraydifference and stores the value in Arraysquared	
